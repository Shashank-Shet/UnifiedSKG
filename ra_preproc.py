...
def codegen_agg(ast_dict):
    ...
    if isinstance(ast_dict, dict):
        ...
        if isinstance(ast_dict[agg_type], dict):
            ...
            elif sec_agg_type == 'value':
                val_node = codegen_agg(ast_dict[agg_type]['value'])
                val_node.parent = agg_type_node
                pass
            else:
                # TODO Add code for <Table.column>
                # print(sec_agg_type)
                # print(ast_dict[agg_type].keys())
                raise Exception(str(sec_agg_type))
        else:
        ...
        raise Exception(f"ra_preproc:198 ast_dict type not scalar or dictionary {type(ast_dict)}")
        print(ast_dict)
...
def ast_to_ra(ast_dict, args=None):
    ...
    select_list = []
    ast_dict_copy = ast_dict
    if ast_dict.get("select") is None and ast_dict.get("select_distinct") is None and ast_dict.get("union") is None and ast_dict.get("intersect") is None and ast_dict.get("from"):
        ast_dict_copy = ast_dict
        ast_dict = ast_dict['from']
    if ast_dict.get("intersect"):
        select_list = ast_dict["intersect"]
        set_op_node = Node("Intersect", n_type="intersect/union/except")
    elif ast_dict.get("union"):
        select_list = ast_dict["union"]
        set_op_node = Node("Union", n_type="intersect/union/except")
    elif ast_dict.get("except"):
        select_list = ast_dict["except"]
        set_op_node = Node("except", n_type="intersect/union/except")
    elif ast_dict.get("select_distinct"):
        ast_dict["select"] = {"value": {"distinct": ast_dict.pop("select_distinct")}}
        select_list = [ast_dict]
        set_op_node = None
    elif ast_dict.get("select"):
        select_list = [ast_dict]
        set_op_node = None
    else:
        raise Exception(f"Unknown init key: {ast_dict.keys()}")
    for i, select_dict in enumerate(select_list):
        root = Node(f"Project_{i}", n_type="Table")
        if select_dict.get("union") or select_dict.get("intersect"):
            node = ast_to_ra(select_dict)
            if set_op_node:
                node.parent = set_op_node
            continue
        if select_dict.get("select_distinct"):
            select_dict["select"] = {"value": {"distinct": select_dict.pop("select_distinct")}}
        select_node = codegen_select(select_dict["select"], args)
        select_node.parent = root
        curr = root

        tables, on_list = codegen_from(select_dict["from"], args)
        where_list = select_dict.get("where")
        having_list = select_dict.get("having")
        condition = codegen_where(where_list, on_list, having_list, args)
        if condition:
            curr = Node("Selection", parent=curr, n_type="Table")
            condition.parent = curr
        tables.parent = curr
        if select_dict.get("groupby"):
            curr = Node("Groupby", n_type="Table")
            codegen_select(select_dict["groupby"], args).parent = curr
            root.parent = curr
            root = curr
        if select_dict.get("orderby"):
            if isinstance(select_dict["orderby"], dict) and select_dict["orderby"].get("sort"):
                sort = "Orderby_" + select_dict["orderby"]["sort"]
            else:
                sort = "Orderby_asc"
            curr = Node(sort, n_type="Table")
            codegen_select(select_dict["orderby"], args).parent = curr
            root.parent = curr
            root = curr
        if select_dict.get("limit"):
            curr = Node("Limit", n_type="Table")
            val = select_dict["limit"]
            ...
            root = root.parent
        if set_op_node:
            root.parent = set_op_node
        pass

    if set_op_node:
        root = set_op_node

    ast_dict = ast_dict_copy
